...
from k12p6_pss_c import k12p6_pss_c_class,c_h_eval,sp_jac_trap_eval,slu2pydae
...
t_0 = time.time()
grid_c.ini(params,xy_0='xy_0.json')
print(time.time()-t_0)
...
def eval_preconditioner_run(self):
    ...
    P_slu_run = spilu(sspa.csc_matrix(self.sp_jac_trap),
...
grid_c.ini(params,xy_0='xy_0.json')
grid_c.ini({'P_1':0e6},xy_0='xy_0.json')

grid_c.Dt = 0.01
grid_c.fill_factor_run = 100
grid_c.drop_tol_run = 1e-4
grid_c.drop_rule_run = 'dynamic,area'
eval_preconditioner_run(grid_c)
t_0 = time.time()
grid_c.sprun(1,{})
grid_c.sprun(60,{'P_1':-600e6})
print(time.time()-t_0)
grid_c.post()
...
grid.ini(params,xy_0='xy_0.json')
grid.Dt = 0.01
grid.fill_factor_run = 100
grid.drop_rule_run = 'basic'
grid.eval_preconditioner_run()
t_0 = time.time()
grid.run(1,{})
grid.run(30,{'P_1':-600e6})
print(time.time()-t_0)
grid.post()
